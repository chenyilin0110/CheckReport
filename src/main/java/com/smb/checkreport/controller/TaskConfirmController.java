...
public class TaskConfirmController {
    ...
    public ResponseEntity<String> checkReport(HttpServletRequest request, Model model, String order, String nest_program_no, String machine_code, String type){
        ...
        try {
            List<ApiLog> get_elementCode = checkReportService.getElementCodeByNestProgramNO(nest_program_no, machine_code, type, request.getSession().getId());
            if(get_elementCode.get(0) == null){
                ar.setRetMessage("Can not find the report!!");
                ar.setRetStatus("Failed");
            }else{
                deleteFile(order + "_" + nest_program_no);//delete checkreport.txt in log folder
...
//                                if(!get_dispatch_detail_sns.get(j).getDispatchDetailSNS().equals("")){
                                if(!stringSplit[k].equals("")){
                                    ...
                                    if(get_order_sn_by_dispath_detail_sns.get(0).getOrderSN().equals(get_order_sn_by_order_num.get(0).getOrderSN())){
                                        ...
                                        writeFile(element_code[i], stringSplit[k], order, nest_program_no);
                                    }else{
                                        ...
                                        writeFile(element_code[i], stringSplit[k], get_order_num_by_order_sn.get(0).getOrderNum(), order, nest_program_no);
                                    }
                                }else{
                                    ...
                                    if(get_order_status.get(0).getOrderStatus().equals("0")){
                                        // 0 is no dispatch
                                        writeFile(element_code[i], order, nest_program_no, 0);
                                        ...
                                        if(getIsFinishedInRelManufactureElementByElementCodeAndStepCodeAndOrderSN.size() == 0){
                                            // have not data in rel_manufacture_element
                                            writeFile(element_code[i], order, nest_program_no, 10);
                                        }else if(getIsFinishedInRelManufactureElementByElementCodeAndStepCodeAndOrderSN.get(0).getIsFinished().equals("1")){
                                            // 1 is mean finish
                                            writeFile(element_code[i], order, nest_program_no, 11);
                                            break;
                                            ...
                                            writeFile(element_code[i], order, nest_program_no, 12);
                                        }
                                    }else if(get_order_status.get(0).getOrderStatus().equals("2")) {
                                        writeFile(element_code[i], order, nest_program_no, 2);
                                    }
                                }
                            }
                        }
                ...
                ar.setRetMessage("Sucess recovery the data");
